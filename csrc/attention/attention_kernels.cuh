...
// Macro for unrolled for loops
#define FOR_UNROLL(init, condition, increment) _Pragma("unroll") for (init; condition; increment)
...
inline __device__ float block_sum(float* red_smem, float sum) {
    ...
    FOR_UNROLL(int mask = WARP_SIZE / 2, mask >= 1, mask /= 2) {
    ...
    FOR_UNROLL(int mask = NUM_WARPS / 2, mask >= 1, mask /= 2) {
        sum += VLLM_SHFL_XOR_SYNC(sum, mask);
    }
...
#pragma unroll
    ...
    for (int block_idx = start_block_idx + warp_idx; block_idx < end_block_idx; block_idx += NUM_WARPS) {
            ...
            if (!is_remote && !is_local) {
                for (int i = 0; i < NUM_TOKENS_PER_THREAD_GROUP; i++) {
                    ...
                    if (thread_group_offset == 0) {
                        // NOTE(linxihui): assign very large number to skipped
                        // tokens to avoid contribution to the sumexp softmax
                        // normalizer. This will not be used at computing
                        // sum(softmax*v) as the blocks will be skipped.
        ...
        // For example, if the thread group size is 4, then the first thread in
        // the group has 0, 4, 8, ... th vectors of the key, and the second
